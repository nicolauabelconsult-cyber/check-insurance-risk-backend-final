...
from security import SecurityHeadersMiddleware  # (mantido se usares noutro ponto)
...
def login(req: LoginReq, request: Request, db: Session = Depends(get_db)):
    ...
    log(str(user.id), "login", {"email": user.email, "ip": request.client.host})
...
def auth_login(payload: LoginReq, request: Request, db: Session = Depends(get_db)):
    return login(req=payload, request=request, db=db)
...
def risk_check(req: RiskCheckReq, payload: dict = Depends(bearer), db: Session = Depends(get_db)):
    ...
    justificacao = (
        rec.justificacao if rec and rec.justificacao
        else ("Histórico limpo; critérios de KYC cumpridos."
              if decisao.startswith("Aceitar")
...
# -----------------------------------------------------------------------------
# Execução local (no Render usa o Start Command)
# -----------------------------------------------------------------------------
