...
def login(body: LoginRequest):
    ...
    Devolve access_token = email, user_name, role.
    """
...
def risk_check(body: RiskCheckRequest, current=Depends(get_current_user)):
    ...
    Se PASSAPORTE / CARTAO_RESIDENTE: força revisão manual.
...
def generate_pdf(consulta_id_raw: int, data: Dict):
    """
    Gera PDF mais profissional, com blocos:
    - Dados da Consulta
    - Resumo de Risco
    - Detalhes Técnicos
    - Histórico e Compliance Interno
    ...
    y = draw_wrapped_line(
        c,
        f"Consulta ID: {data.get('consulta_id_display', '')}",
        40, y, 500
    )
    y = draw_wrapped_line(
        c,
        f"ID Interno: {data.get('consulta_id_raw', '')}",
        40, y, 500
    )
    y = draw_wrapped_line(
        c,
        f"Data/Hora (UTC): {data.get('timestamp', '')}",
        40, y, 500
    )
    y = draw_wrapped_line(
        c,
        f"Identificador Analisado: {data.get('identifier', '')}",
        40, y, 500
    )
    y = draw_wrapped_line(
        c,
        f"Tipo de Identificador: {data.get('identifier_type', '')}",
        40, y, 500
    )
    y -= 10
    ...
    y = draw_wrapped_line(
        c,
        f"Score Final: {score_final_val}/100",
        40, y, 500
    )
    y = draw_wrapped_line(
        c,
        f"Decisão Recomendada: {decisao_val}",
        40, y, 500
    )
    y = draw_wrapped_line(
        c,
        f"Alerta PEP / Exposição Política: {pep_txt}",
        40, y, 500
    )
    y = draw_wrapped_line(
        c,
        f"Alerta Sanções Internacionais: {sanc_txt}",
        40, y, 500
    )
    y -= 10
    ...
    y = draw_wrapped_line(
        c,
        f"Justificação Técnica: {justificacao_val}",
        40, y, 500
    )
    y = draw_wrapped_line(
        c,
        f"Benchmark / Referência: {benchmark_val}",
        40, y, 500
    )
    y -= 10
    ...
    for rec in RISK_DATA_STORE:
        # tentamos casar o identificador com nif, bi, passaporte, cartão de residente ou nome
    ...
    if match_hist:
        ...
        y = draw_wrapped_line(
            c,
            f"Pagamentos: {hist_pag}",
            40, y, 500
        )
        y = draw_wrapped_line(
            c,
            f"Sinistros Totais: {sin_tot}",
            40, y, 500
        )
        y = draw_wrapped_line(
            c,
            f"Sinistros Últimos 12 Meses: {sin_12m}",
            40, y, 500
        )
        y = draw_wrapped_line(
            c,
            f"Fraude Suspeita: {fraude_flag}",
            40, y, 500
        )
        if fraude_com:
            y = draw_wrapped_line(
                c,
                f"Nota Fraude: {fraude_com}",
                40, y, 500
            )
    else:
        y = draw_wrapped_line(
            c,
            "Sem histórico interno associado a este identificador.",
            40, y, 500
        )
