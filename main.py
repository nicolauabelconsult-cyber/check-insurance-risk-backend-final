...
from fastapi import FastAPI, Depends, HTTPException, UploadFile, File, Form, status, Query, Header
...
app = FastAPI(title="Check Insurance Risk Backend", version="2.0.0")
...
@app.get("/")
def root():
    return {"ok": True, "service": "CIR Backend", "version": "2.0.0"}
...
def get_report(consulta_id: str, token: str = Query(None), authorization: str | None = Header(default=None)):
    # Aceita query ?token=... ou header Authorization
    if token:
        decode_token(token)
    elif authorization:
...
def admin_risk_add_record(
    id: str = Form(None),
    nome: str = Form(None), nif: str = Form(None), bi: str = Form(None),
    passaporte: str = Form(None), cartao_residente: str = Form(None),
    score_final: int = Form(0), justificacao: str = Form(None),
    pep_alert: str = Form("0"), sanctions_alert: str = Form("0"),
    historico_pagamentos: str = Form(None), sinistros_total: int = Form(0),
    sinistros_ult_12m: int = Form(0), fraude_suspeita: str = Form("0"),
    comentario_fraude: str = Form(None), esg_score: int = Form(0),
    country_risk: str = Form(None), credit_rating: str = Form(None),
...
def admin_risk_list(payload: dict = Depends(bearer), db: Session = Depends(get_db)):
    ...
    def ser(r: RiskRecord):
        return {
            "id": r.id, "nome": r.nome, "nif": r.nif, "bi": r.bi,
            "passaporte": r.passaporte, "cartao_residente": r.cartao_residente,
            "score_final": r.score_final, "justificacao": r.justificacao,
            "pep_alert": r.pep_alert, "sanctions_alert": r.sanctions_alert,
            ...
            "esg_score": r.esg_score, "country_risk": r.country_risk,
            "credit_rating": r.credit_rating, "kyc_confidence": r.kyc_confidence
        }
...
def info_source_create(
    ...
    url: str = Form(None), directory: str = Form(None), filename: str = Form(None),
    categoria: str = Form(None), source_owner: str = Form(None), validade: str = Form(None),
    payload: dict = Depends(bearer), db: Session = Depends(get_db)
):
    if payload.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Apenas administradores")
