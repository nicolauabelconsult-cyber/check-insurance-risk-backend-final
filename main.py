...
def risk_check(req: RiskCheckReq, payload: dict = Depends(bearer), db: Session = Depends(get_db)):
    # Basic heuristic scoring demo
    base_score = random.randint(60, 92)
    pep = "PEP" in req.identifier.upper() or random.random() < 0.1
    sanc = random.random() < 0.05
    fraude = random.random() < 0.08
    ...
    # Try to match a manual record if exists by any identifier
    rec = db.query(RiskRecord).filter(
        (RiskRecord.nif == req.identifier) | (RiskRecord.bi == req.identifier) | 
        (RiskRecord.passaporte == req.identifier) | (RiskRecord.cartao_residente == req.identifier)
    ).first()
    ...
    resp = dict(
        ...
        consulta_id_raw=consulta_id,
        timestamp=dt.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC"),
        identifier=req.identifier,
        identifier_type=req.identifier_type,
        score_final=rec.score_final if rec else base_score,
        decisao=decisao if not rec else ("Aceitar" if (rec.score_final or 0) >= 80 else "Escalar para revisão manual"),
        justificacao=rec.justificacao if rec and rec.justificacao else justificacao,
        pep_alert=bool(rec.pep_alert) if rec else pep,
        sanctions_alert=bool(rec.sanctions_alert) if rec else sanc,
        ...
        fraude_suspeita=rec.fraude_suspeita if rec else fraude,
...
def get_report(consulta_id: str, token: str = Query(None)):
    # validate token via query param for compatibility with the frontend
    if not token:
        raise HTTPException(status_code=401, detail="Token ausente")
    payload = require_token(token)

    # In a real app we'd fetch stored result by consulta_id. Here we render a generic PDF.
    meta = {
        "consulta_id": consulta_id,
        "timestamp": dt.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC"),
        "identifier": "—",
...
def admin_risk_add_record(
    ...
    pep_bool = (str(pep_alert) in ("1","true","True"))
    sanc_bool = (str(sanctions_alert) in ("1","true","True"))
    fraude_bool = (str(fraude_suspeita) in ("1","true","True"))
    ...
    for k, v in dict(
        ...
        score_final=score_final, justificacao=justificacao, pep_alert=pep_bool,
        sanctions_alert=sanc_bool, historico_pagamentos=historico_pagamentos,
        sinistros_total=sinistros_total, sinistros_ult_12m=sinistros_ult_12m,
        fraude_suspeita=fraude_bool, comentario_fraude=comentario_fraude,
        esg_score=esg_score, country_risk=country_risk, credit_rating=credit_rating,
...
def admin_risk_list(payload: dict = Depends(bearer), db: Session = Depends(get_db)):
    ...
    def serialize(r: RiskRecord):
        return {
            "id": r.id, "nome": r.nome, "nif": r.nif, "bi": r.bi, "passaporte": r.passaporte,
            "cartao_residente": r.cartao_residente, "score_final": r.score_final, "justificacao": r.justificacao,
            "pep_alert": r.pep_alert, "sanctions_alert": r.sanctions_alert, "historico_pagamentos": r.historico_pagamentos,
            "sinistros_total": r.sinistros_total, "sinistros_ult_12m": r.sinistros_ult_12m,
            "fraude_suspeita": r.fraude_suspeita, "comentario_fraude": r.comentario_fraude,
            "esg_score": r.esg_score, "country_risk": r.country_risk, "credit_rating": r.credit_rating,
...
def info_source_create(
    ...
    item = InfoSource(
        title=title, description=description, url=url, directory=directory, filename=filename,
...
def info_source_list(payload: dict = Depends(bearer), db: Session = Depends(get_db)):
    ...
    return [{
        "title": r.title, "description": r.description, "url": r.url, "directory": r.directory,
...
def info_source_delete(index: int = Form(...), payload: dict = Depends(bearer), db: Session = Depends(get_db)):
    ...
    if index < 0 or index >= len(rows):
        raise HTTPException(status_code=404, detail="Index inválido")
    db.delete(rows[index]); db.commit()
    return {"status": "deleted"}

# ---------- Healthcheck ----------
@app.get("/")
def root():
    return {"ok": True, "service": "CIR Backend", "version": "1.0.0"}
