...
from models import User, RiskRecord, InfoSource, ReportCache
...
app = FastAPI(title="Check Insurance Risk Backend", version="3.1.0")
...
def risk_check(req: RiskCheckReq, payload: dict = Depends(bearer), db: Session = Depends(get_db)):
    ...
    rc = ReportCache(consulta_id=consulta_id, payload=json.dumps(payload_data))
    db.add(rc); db.commit()
    return payload_data
...
def get_report(consulta_id: str, token: str = Query(None), authorization: str | None = Header(default=None),
    ...
    row = db.query(ReportCache).filter(ReportCache.consulta_id==consulta_id).first()
    if row: meta = json.loads(row.payload)
    else:
...
def admin_user_list(payload: dict = Depends(bearer), db: Session = Depends(get_db)):
    if payload.get("role") != "admin": raise HTTPException(status_code=403, detail="Apenas administradores")
    rows = db.query(User).order_by(User.id).all()
    return [{"id":u.id,"name":u.name,"email":u.email,"role":u.role,"created_at":str(u.created_at)} for u in rows]
...
def admin_user_add(new_name: str = Form(...), new_email: str = Form(...), new_password: str = Form(...),
    ...
    db.add(u); db.commit(); db.refresh(u); return {"status":"ok","user":{"id":u.id,"email":u.email,"role":u.role}}

@app.post("/api/admin/user-delete")
def admin_user_delete(user_id: int = Form(...), payload: dict = Depends(bearer), db: Session = Depends(get_db)):
    if payload.get("role") != "admin": raise HTTPException(status_code=403, detail="Apenas administradores")
    me = int(payload.get("sub"))
    if user_id == me: raise HTTPException(status_code=400, detail="Não pode remover a própria conta")
    u = db.query(User).filter(User.id==user_id).first()
    if not u: raise HTTPException(status_code=404, detail="Utilizador não encontrado")
    if u.role == "admin":
        admins = db.query(User).filter(User.role=="admin").count()
        if admins <= 1: raise HTTPException(status_code=400, detail="Não pode remover o último administrador")
    db.delete(u); db.commit(); return {"status":"deleted"}
