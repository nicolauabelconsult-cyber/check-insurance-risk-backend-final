...
def create_info_source(
    ...
    cur.execute(
        """
        INSERT INTO info_sources (title, description, url, directory, filename, uploaded_at)
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (
            title,
    ...
    conn.commit()
    conn.close()
    return {"status": "ok", "message": "Fonte registada"}

@router.get("/info-sources/list")
def list_info_sources(admin=Depends(require_admin)):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        """
        SELECT id, title, description, url, directory, filename, uploaded_at
        FROM info_sources
        ORDER BY id DESC
        """
    )
