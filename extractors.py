...
def extract_names_roles(html: str) -> List[Dict[str, Any]]:
    ...
    for tag in soup.find_all(["h1","h2","h3","h4"]):
        ...
        if len(txt.split()) >= 2 and re.search(NAME_PATTERN, txt):
            role = None
            mrole = re.search(PEP_TITLES, txt, flags=re.I)
...
def extract_claims(text: str) -> Dict[str,int]:
    ...
    auto_hits = len(re.findall(r"\b(auto|autom[oó]vel|viatura)\b.*\b(sinistro|acidente)\b", t))
    vida_hits = len(re.findall(r"\b(vida)\b.*\b(sinistro|ocorr[eê]ncia)\b", t))
    saude_hits = len(re.findall(r"\b(sa[uú]de)\b.*\b(sinistro|ocorr[eê]ncia)\b", t))
    patr_hits = len(re.findall(r"\b(patrimoni(al|o))\b.*\b(sinistro|ocorr[eê]ncia)\b", t))
...
def extract_payments(text: str) -> Dict[str,Any]:
    ...
    last_delay = None
    m = re.search(r"(jan|fev|mar|abr|mai|jun|jul|ago|set|out|nov|dez)\s*[\-/]?\s*(20\d{2})", t, flags=re.I)
...
def extract_facts_from_sources(sources: List[Dict[str,Any]]) -> Dict[str,Any]:
    ...
    punctuality = payments["punctuality_pct"] or 80
    if punctuality < 85: base -= 10
    ...
    facts = {
        ...
        "justificacao": "Factos agregados automaticamente a partir das fontes; recomenda-se validação humana quando confidence < 0.8."
    }
    return facts
