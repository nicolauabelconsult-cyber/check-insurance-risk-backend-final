...
def get_report(consulta_id: str, token: str = Query(None)):
    """
    token é passado só para manter compatibilidade com o frontend (?token=...).
    Em produção, deve validar JWT. Aqui é apenas demo.
    """
...
def build_pdf(analysis: dict) -> BytesIO:
    ...
    # HEADER
    header_height = 22 * mm
    p.setFillColorRGB(0.07, 0.1, 0.18)  # dark header
