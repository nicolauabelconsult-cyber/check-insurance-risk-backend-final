...
def get_report(consulta_id: str, token: str = Query(None)):
    # token é ignorado neste mock, apenas para compatibilidade com o front
...
def gerar_resumo_operacional(a: dict):
    linhas = []
    hp = a.get("historico_pagamentos") or "Sem registo"
    linhas.append(f"Pagamentos: {hp}.")
    total = int(a.get("sinistros_total",0))
    ult12 = int(a.get("sinistros_ult_12m",0))
    linhas.append(
        f"Sinistros registados: {total} no total, {ult12} nos últimos 12 meses."
    )
    if a.get("fraude_suspeita"):
        comentario = a.get("comentario_fraude") or "Sem detalhe adicional."
        linhas.append("Indicador de possível fraude / abuso operacional.")
        linhas.append(f"Nota interna: {comentario}")
    else:
        linhas.append("Não existem indicadores activos de fraude.")
    return linhas
...
def build_pdf(analysis: dict) -> BytesIO:
    ...
    comp_hist = comportamento_historico_label(analysis)
    ...
    if analysis.get("fraude_suspeita"):
        flags_txt.append("Suspeita de Fraude")
    flags_line = "Sem alertas PEP / Sanções / Fraude" if not flags_txt else "Alertas: " + " / ".join(flags_txt)
    ...
    p.drawString(
        margin_left + 6 * mm,
        box_y + box_h - 25.5 * mm,
        "Comportamento Histórico: " + comp_hist
    )
    ...
    detalhes = [
        ...
        ("Fraude Suspeita", "SIM" if analysis.get("fraude_suspeita") else "NÃO"),
        ("Histórico de Pagamentos", analysis.get("historico_pagamentos","-")),
        ("Sinistros Totais", str(analysis.get("sinistros_total","-"))),
        ("Sinistros Últimos 12 Meses", str(analysis.get("sinistros_ult_12m","-"))),
        ("Nota de Fraude / Abuso", analysis.get("comentario_fraude","-")),
    ]
    ...
    p.setFillColor(colors.black)
    p.setFont("Helvetica-Bold", 10)
    p.drawString(margin_left, cursor_y, "Histórico de Comportamento e Sinistros")
    cursor_y -= 6 * mm

    p.setFont("Helvetica", 8.5)
    p.setFillColor(colors.black)

    narrativa_lines = gerar_resumo_operacional(analysis)
    for line in narrativa_lines:
        if cursor_y < 30 * mm:
            _draw_footer(p)
            p.showPage()
            cursor_y = PAGE_H - 25 * mm
    ...
    obs_lines = [
        ...
        "elevado, suspeita de fraude ou quando existam alertas PEP / sanções.",
        "",
        "Assinatura técnica:",
