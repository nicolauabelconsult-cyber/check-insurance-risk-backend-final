...
def next_user_id(): return next(_user_id_counter)
def next_risk_id(): return next(_risk_id_counter)
def next_source_id(): return next(_source_id_counter)
def next_consulta_id(): return str(next(_consulta_id_counter))
def timestamp_now(): return datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
